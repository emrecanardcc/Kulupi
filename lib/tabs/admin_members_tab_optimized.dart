...
class _AdminMembersTabState extends State<AdminMembersTab> {
  ...
  Future<void> _changeMemberRole(String userId, String newRole) async {
    try {
      await _databaseService.updateMemberRole(
        int.parse(widget.kulupId),
        userId,
        AppRole.values.firstWhere((r) => r.toJson() == newRole),
      );
    } catch (e) {
      if (mounted) {
  ...
  String _getMemberName(Map<String, dynamic> member) {
    ...
    if (profile == null) return member['user_id']?.toString() ?? 'Bilinmeyen';
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<Map<String, dynamic>>>(
      ...
      builder: (context, snapshot) {
        ...
        // OPTIMIZATION: Load all profiles at once instead of N+1 queries
        return FutureBuilder<List<Map<String, dynamic>>>(
          future: _databaseService.getClubMembersWithProfiles(int.parse(widget.kulupId)),
          builder: (context, profilesSnapshot) {
            if (!profilesSnapshot.hasData) {
              return const Center(child: CircularProgressIndicator());
            }

            final membersWithProfiles = profilesSnapshot.data!;
            ...
            return ListView.separated(
              ...
              itemBuilder: (context, index) {
                ...
                String role = member['role'] ?? 'uye';
                final Color onSurface = Theme.of(context).colorScheme.onSurface;
                var style = _getRoleStyle(context, role);
                final roleColor = style['color'] as Color;
                final String name = _getMemberName(member);

                return GestureDetector(
                  onTap: () => _showRoleDialog(member['user_id'], name, role),
                  child: AuraGlassCard(
