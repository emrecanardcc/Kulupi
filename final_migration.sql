-- GÜVENLİ MİGRASYON SCRIPT'İ - NİHAİ VERSİYON
-- Tüm syntax hataları düzeltilmiş
...
ALTER TABLE public.club_members DROP COLUMN IF EXISTS role;
ALTER TABLE public.club_members DROP COLUMN IF EXISTS status;

-- ADIM 5: Yeni sütunları rename et
ALTER TABLE public.club_members RENAME COLUMN role_new TO role;
ALTER TABLE public.club_members RENAME COLUMN status_new TO status;

-- ADIM 6: Default değerleri ayarla
ALTER TABLE public.club_members ALTER COLUMN role SET DEFAULT 'uye'::public.club_role;
ALTER TABLE public.club_members ALTER COLUMN role SET NOT NULL;
ALTER TABLE public.club_members ALTER COLUMN status SET DEFAULT 'pending'::public.membership_status;
ALTER TABLE public.club_members ALTER COLUMN status SET NOT NULL;
...
ALTER TABLE public.clubs RENAME COLUMN status_new TO status;

-- Default değerleri ayarla
ALTER TABLE public.clubs ALTER COLUMN status SET DEFAULT 'active'::public.club_status;
...
-- ADIM 9: Unique constraint ekle - DÜZELTİLMİŞ
-- Önce varsa eski constraint'i kaldır
ALTER TABLE public.club_members DROP CONSTRAINT IF EXISTS unique_club_user;
